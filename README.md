...
model. **Phase 2** (this phase) adds a transparent, trainable, backtestable baseline model on top
of that architecture — see [MODEL_CARD.md](./MODEL_CARD.md) for its intended use, design, and
limitations before reading anything into its numbers. **The bundled model is trained on synthetic
data and is explicitly labelled `SYNTHETIC TEST MODEL — NOT FOR BETTING USE` everywhere it
appears in the UI.**
...
python/
  ...
  tests/                  pytest suite, including the explicit anti-leakage test
src/
...
npm run model:test  # Python (pytest) — 59 tests, incl. the leakage test
```
...
1. Replace synthetic history with a licensed/real data feed once available; retrain and re-run the
   full evaluation before trusting any number in this app.
2. Hyperparameter search (currently baseline defaults) and feature selection, still governed by
   validation-only tuning — never test.
...
5. Wire the Lucky 15 leg-selection algorithm (expected value + place probability + confidence +
   enhanced place terms + diversification — Phase 3/4, explicitly out of scope here).
6. Consider a Postgres migration once multi-user or hosted deployment is in scope.
7. Only after (1) and genuine out-of-sample evidence of edge: staking logic. Not before.
